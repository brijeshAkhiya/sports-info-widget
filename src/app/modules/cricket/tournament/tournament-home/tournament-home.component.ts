...
export class TournamentHomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.options = {
      reqParams: {
        aIds: [this.commonService.getIds(this.activatedroute.snapshot.params.id, 'cricket', 'tournament')],
        eSport: 'Cricket'
      }, title: name.replace(/-/g, ' '), id: this.tournamentid, name: name.replace(/-/g, ' '), type: 'tournament', sport: 'cricket'
    };
